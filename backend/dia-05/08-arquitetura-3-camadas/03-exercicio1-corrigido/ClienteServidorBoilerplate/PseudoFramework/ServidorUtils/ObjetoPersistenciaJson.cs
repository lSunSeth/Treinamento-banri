...
{
    public class ObjetoPersistenciaJson<T> where T : class, new()
    {
        ...
        public ObjetoPersistenciaJson(string arquivo)
        {
            ...
            var listaDicionarios = ConversorJson.Desserializar<List<Dictionary<string, object>>>(conteudo);

            if (listaDicionarios == null)
            {
                _itens = new List<T>();
            ...
            _itens = listaDicionarios
                .Select(dicionario => CampoArquivoJsonMapper.FromDicionario<T>(dicionario))
                .ToList();
        }
        ...
        private void Salvar()
        {
            var listaDicionarios = _itens
                .Select(item => CampoArquivoJsonMapper.ToDicionario(item))
                .ToList();

            File.WriteAllText(_arquivo, ConversorJson.Serializar(listaDicionarios));
        }
    }
}
